...
def evaluate_class(model, data_loader, criterion, device, logger) \
    ...
    Returns:
        tuple: 包含以下元素的元组：
            ...
            - aa (float): 平均准确率 (Average Accuracy)。
            - kappa (float): Kappa 系数。
            - class_accuracies (numpy.ndarray): 所有预测标签的数组。
    """
    model.eval()
    running_loss = 0.0
    all_preds = []
    ...
    return avg_loss, overall_accuracy, aa, kappa, class_accuracies
